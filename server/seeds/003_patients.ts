	...
	for (const company of companies) {
		...
		for (let i = 0; i < count; i++) {
			...
			batch.push({
				...
				date_of_birth: faker.helpers.maybe(
					() => faker.date.birthdate({ min: 1, max: 90, mode: "age" }),
					{ probability: 0.9 },
				),
				gender,
				occupation: faker.helpers.maybe(() => faker.person.jobTitle(), {
					probability: 0.5,
				}),
				...
				medical_info: faker.helpers.maybe(
					() => ({
						...
						current_medications: faker.helpers.maybe(
							() => faker.lorem.words({ min: 1, max: 3 }),
							{ probability: 0.3 },
						),
						surgeries_history: faker.helpers.maybe(
							() => faker.lorem.words({ min: 1, max: 3 }),
							{ probability: 0.2 },
						),
						medical_history: faker.helpers.maybe(() => faker.lorem.sentence(), {
							probability: 0.4,
						}),
					}),
					{ probability: 0.4 },

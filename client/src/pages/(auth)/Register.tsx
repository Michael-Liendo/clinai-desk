...
export default function RegisterPage() {
  ...
  const { values, errors, handleChange, handleSubmit, isSubmitting, setFieldValue } =
    useFormik({
      initialValues: {
        first_name: "",
        last_name: "",
      ...
      onSubmit: async (vals) => {
        try {
          ...
          const companyName = vals.company_name?.trim() || `Consultorio de ${vals.first_name} ${vals.last_name}`.trim();
          const createdCompany = await Services.companies.create({
            name: companyName,
            email: vals.email,
            address: vals.company_address || "N/A",
            phone: vals.company_phone || "N/A",
          });
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2">
      <div className="flex flex-col justify-center items-center p-8">
        ...
        <br />
        <p className="text-lg mb-10 text-center">Crea tu cuenta como médico independiente</p>
          ...
          <CardContent>
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <TextField
                  ...
                  onChange={(e) => {
                    ...
                    const f = e.target.name === "first_name" ? e.target.value : values.first_name;
                    const l = e.target.name === "last_name" ? e.target.value : values.last_name;
                    const suggested = `Consultorio de ${f} ${l}`.trim();
                    if (!values.company_name) setFieldValue("company_name", suggested);
                  }}
                />
                <TextField
                  type="text"
              ...
              <div className="text-center text-sm text-muted-foreground">
                ¿Ya tienes cuenta? {" "}
                <Link to={AuthRoutesEnum.Login} className="text-primary underline">
                  Inicia sesión
                </Link>
              </div>
            </form>
